...
(function($){
	$.litelighter = function($this, options){
		this.settings = $.extend({},{ clone: false, style: 'light', language: 'html', tab: '    ' },options);
		this.code = $this;
		this.enable();
	};
	...
	$.extend($.litelighter.prototype, {
		enable: function(){
				...
				if(this.settings.clone == true)
					this.codelite = $('<div />').text(this.code.text()).addClass('litelighter').insertAfter(this.code.css('display','none'));
