...
class Cowobo_Feed {
	...
	public function __construct($args, $wp_query = false) {
		...
		// If we get no query or got no results, return the latest posts [temporary]
		if ( empty($this->feedposts) ) {
			$exclude = '-'.get_cat_ID('Uncategorized').',-'.get_cat_ID('Coders').',-'.get_cat_ID('Locations');
			$this->feedposts = get_posts(array('numberposts'=>'10', 'cat'=>$exclude));
		}
		
		// Set the description and titles
		$this->feed_title = "Coders Without Borders";
	...
	function get_post_feed( $postid ) {
		...
		$newposts = array(get_post($postid));
	...
	function add_image($postid) {
		$img = cwb_get_first_image($postid);
		if($img) {
			$image = '<img src="'. $img .'"/>';
			echo $image;
		}
