...
elseif ($_POST['userpw']):
	...
	echo '<form method="post" action="?confirm=true">';
		echo '<input type="hidden" name="userpw" value="'.$_POST['userpw'].'"/>';
		echo '<input type="hidden" name="username" value="'.$_POST['username'].'"/>';			
		echo '<button type="submit" class="button">Yes, add me</button>';
		echo '<a class="button" href="?action=login&relogin='.$_POST['username'].'">No, I have logged in before</a>';
	echo '</form>';
	...
	echo '<form method="post" action="?action=login">';
		echo '<input type="text" name="username" class="smallfield" value="'.$default.'" onfocus="this.value=\'\'" onblur="if(this.value==\'\') this.value=\'ie John\'" />';
